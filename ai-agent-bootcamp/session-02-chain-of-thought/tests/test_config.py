"""Tests for Session 02 Chain of Thought — config and import validation."""
import pytest
import os
import pathlib
...
def test_renamed_health_csv_file_exists():
    """Renamed file (space -> underscore) must exist."""
    project_root = pathlib.Path(__file__).parent.parent
    renamed = project_root / "8.1.Example8-Health_care_read_from_csv.py"
    assert renamed.exists(), "Renamed health care CSV file not found"
...
def test_no_bare_except_clauses():
    ...
    for py_file in project_root.glob("*.py"):
        ...
        if bare_except.search(content):
            violations.append(py_file.name)
    assert not violations, f"Bare except: clauses found in: {violations}"
