"""Tests for Session 04 Text-to-SQL — config and import validation."""
import pytest
import os
import pathlib
...
def test_no_hardcoded_key_comment_in_txt2sql():
    """txt2sql.py must not contain the old hardcoded key comment."""
    project_root = pathlib.Path(__file__).parent.parent
    txt2sql = project_root / "txt2sql.py"
    assert txt2sql.exists(), "txt2sql.py not found"
    content = txt2sql.read_text()
    assert 'Replace this with your open ai key' not in content, \
        "Hardcoded key comment still present in txt2sql.py"
