"""
Tests for Neural Style Transfer — validates config loading and key validation.
Run: pytest tests/
"""
import pytest
import os
...
def test_gpt_demo_paths_fixed():
    """gpt_demo.py should use content-images/ and style-images/ directories."""
    import pathlib
    gpt = pathlib.Path(__file__).parent.parent / "gpt_demo.py"
    content = gpt.read_text()
    assert 'CONTENT_PATH = "content.jpg"' not in content, "Old hardcoded content path still present"
    assert 'STYLE_PATH   = "style.jpg"' not in content, "Old hardcoded style path still present"
