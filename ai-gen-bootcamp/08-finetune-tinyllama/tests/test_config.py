"""
Tests for Fine-tune TinyLlama — validates config and file structure.
Run: pytest tests/
"""
import pytest
import os
...
def test_env_loads_without_error(monkeypatch):
    ...
    monkeypatch.setenv("HUGGINGFACE_TOKEN", "hf_test-key-12345")
...
def test_typo_filename_fixed():
    """infer_with_adpter_legal.py (typo) should not exist; correct name should."""
    import pathlib
    root = pathlib.Path(__file__).parent.parent
    assert not (root / "infer_with_adpter_legal.py").exists(), "Typo filename still present"
    assert (root / "infer_with_adapter_legal.py").exists(), "Corrected filename not found"
