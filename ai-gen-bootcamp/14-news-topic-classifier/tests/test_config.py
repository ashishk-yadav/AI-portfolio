"""
Tests for News Topic Classifier — validates config loading and key validation.
Run: pytest tests/
"""
import pytest
import os
...
def test_processed_csvs_not_committed():
    """data/processed/*.csv should not be present (generated artifacts)."""
    import pathlib
    processed = pathlib.Path(__file__).parent.parent / "data" / "processed"
    if processed.exists():
        csvs = list(processed.glob("*.csv"))
        assert len(csvs) == 0, f"Generated CSVs present: {csvs}"
