"""
Tests for Marketing Copy Generator — validates config loading and key validation.
Run: pytest tests/
"""
import pytest
import os
...
def test_runs_log_not_present():
    """runs_log.csv should not be committed (runtime artifact)."""
    import pathlib
    runs_log = pathlib.Path(__file__).parent.parent / "runs_log.csv"
    assert not runs_log.exists(), "runs_log.csv should not be in source control"
